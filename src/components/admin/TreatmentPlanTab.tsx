...
import { getPatients, getTreatments, saveTreatments, completeTreatmentSession, recalculatePatientBalances, addNotification } from '../../clinicDb';
...
export default function TreatmentPlanTab({ initialPatientId, triggerRefresh }: TreatmentPlanTabProps) {
  ...
  const handleDeletePlan = (planId: string) => {
    if (window.confirm('هل أنت متأكد من حذف خطة العلاج وجلساتها بالكامل من النظام؟')) {
      const list = getTreatments();
      const filtered = list.filter(t => t.id !== planId);
