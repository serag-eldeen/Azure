...
import { 
  ...
  addNotification, calculateProfitSharingReport, PeriodFilter, isDateInPeriod, getSettlements, saveSettlement, saveMonthLock
} from '../../clinicDb';
...
export default function FinanceTab({ triggerRefresh }: FinanceTabProps) {
  ...
  const handleDeleteExpense = (id: string) => {
    if (window.confirm('هل أنت متأكد من حذف هذا المصروف نهائياً من السجلات؟')) {
      const currentList = getExpenses();
  ...
  const getUserEmail = (): string => {
    ...
    try {
      ...
      const parsed = JSON.parse(jsonPayload);
      return parsed.email || 'admin@example.com';
    } catch (e) {
