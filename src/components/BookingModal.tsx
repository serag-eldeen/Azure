...
export default function BookingModal({ isOpen, onClose, onBookingSuccess, initialServiceId }: BookingModalProps) {
  ...
  const availableTimes = [
    { value: '02:30', label: '2:30 مساءً' },
    { value: '03:30', label: '3:30 مساءً' },
    { value: '04:30', label: '4:30 مساءً' },
    { value: '05:30', label: '5:30 مساءً' },
    { value: '06:30', label: '6:30 مساءً' },
    { value: '07:30', label: '7:30 مساءً' },
    { value: '08:30', label: '8:30 مساءً' },
    { value: '09:30', label: '9:30 مساءً' },
  ];
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newAppointment: Appointment = {
      ...
      doctor: selectedDoctor ? selectedDoctor.nameAr : 'كبير الأطباء المختصين',
      date,
      time: availableTimes.find(t => t.value === time)?.label || time,
    ...
    // Also sync with admin appointments database!
    try {
      ...
      if (!adminPatient) {
        ...
        const newPatId = activePatientId || ('PAT-' + Math.floor(100000 + Math.random() * 900000));
        adminPatient = {
          id: newPatId,
          name: name.trim(),
          phone: phone.trim(),
      ...
      const docObj = doctorsList.find(d => d && d.name === (selectedDoctor?.nameAr || ''));
      const docEmail = docObj ? docObj.email : 'amr.eladawy@gmail.com';

      const newAdminApp: AdminAppointment = {
        id: newAppointment.id, // match the exact same ID so they are synced!
        patientId: adminPatient.id,
        patientName: name.trim(),
        patientPhone: phone.trim(),
        doctorEmail: docEmail,
        doctorName: selectedDoctor ? selectedDoctor.nameAr : 'كبير الأطباء المختصين',
        date,
        time: availableTimes.find(t => t.value === time)?.label || time,
        duration: 30, // default
