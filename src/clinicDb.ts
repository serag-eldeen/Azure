...
async function attemptTokenRefresh(): Promise<boolean> {
  const refreshToken = localStorage.getItem("azure_refresh_token");
...
async function syncWithBackend(key: string, data: any) {
  try {
    ...
    if (res.status === 401) {
      ...
      if (refreshed) {
        const newToken = sessionStorage.getItem("azure_access_token");
        if (newToken) {
...
export async function initClinicDb(): Promise<void> {
  if (isDbLoadedState) return;
  ...
  try {
    ...
    const [
      ...
      fetchSafe("/api/settlements"),
      fetchSafe("/api/profit-transactions"),
  ...
  // Local storage fallback for reliability (e.g. static rendering, compile checks)
  const keysArray = [
...
export function recalculatePatientBalances() {
  ...
  const updatedPatients = patients.map(p => {
    ...
    const patInvoices = invoices.filter(inv => inv.patientId === p.id && inv.status !== 'Cancelled');
    ...
    const patTreatments = treatments.filter(t => t.patientId === p.id && t.status !== 'Cancelled');
    ...
    const outstandingBalance = invoiceOutstanding;
...
export function calculateProfitSharingReport(filter: PeriodFilter): MonthlyProfitReport {
  ...
  completedApptsWithoutInvoice.forEach(appt => {
    ...
    if (email === 'amr.eladawy@gmail.com' || email === 'basel.elmorsy@gmail.com') {
      report.distributions[email].appointmentRevenue += fee * 0.65;
      report.distributions[partnerEmail].appointmentRevenue += fee * 0.35;
    } else {
      // fallback
      report.distributions['amr.eladawy@gmail.com'].appointmentRevenue += fee * 0.50;
      report.distributions['basel.elmorsy@gmail.com'].appointmentRevenue += fee * 0.50;
    }
  });
  ...
  invoices.forEach(inv => {
    ...
    if (inv.treatmentPlanId) {
      ...
      const patientAppointments = appointments.filter(app => app.patientId === inv.patientId);
      ...
      if (docEmail === 'amr.eladawy@gmail.com' || docEmail === 'basel.elmorsy@gmail.com') {
        report.distributions[docEmail].treatmentRevenue += paidInPeriod * 0.65;
        report.distributions[partnerEmail].treatmentRevenue += paidInPeriod * 0.35;
      } else {
        report.distributions['amr.eladawy@gmail.com'].treatmentRevenue += paidInPeriod * 0.50;
        report.distributions['basel.elmorsy@gmail.com'].treatmentRevenue += paidInPeriod * 0.50;
      }
      ...
      if (appt && appt.status === 'Completed') {
        ...
        if (docEmail === 'amr.eladawy@gmail.com' || docEmail === 'basel.elmorsy@gmail.com') {
          report.distributions[docEmail].appointmentRevenue += paidInPeriod * 0.65;
          report.distributions[partnerEmail].appointmentRevenue += paidInPeriod * 0.35;
        } else {
          report.distributions['amr.eladawy@gmail.com'].appointmentRevenue += paidInPeriod * 0.50;
          report.distributions['basel.elmorsy@gmail.com'].appointmentRevenue += paidInPeriod * 0.50;
        }
      }
    } else {
